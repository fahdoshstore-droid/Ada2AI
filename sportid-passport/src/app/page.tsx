      ...
      <header className="sticky top-0 z-40 border-b border-white/5 bg-[#000A0F]/95 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-0.5 text-sm" ref={dropRef}>
            <Link href="/" className="px-3.5 py-2 rounded-lg text-[13px] font-cairo text-white bg-[#00DCC8]/10 border border-[#00DCC8]/20 select-none">
              {lang === 'ar' ? 'المنتج' : 'Product'}
            </Link>
            <Link href="/hub" className="px-3.5 py-2 rounded-lg text-[13px] font-cairo text-white/45 hover:text-white/80 hover:bg-white/5 transition-all select-none">
              {lang === 'ar' ? 'مركز التدريب' : 'Training Hub'}
            </Link>
            <Link href="/partnerships" className="px-3.5 py-2 rounded-lg text-[13px] font-cairo text-white/45 hover:text-white/80 hover:bg-white/5 transition-all select-none">
            ...
            <div className="relative">
              ...
              {dropdown === 'gov' && (
                <div className="absolute top-full mt-1 left-0 w-52 rounded-xl border overflow-hidden z-50 py-1"
                  ...
                  {[
                    { en: 'Data Governance',   ar: 'حوكمة البيانات',   href: '/governance' },
                    { en: 'Compliance',         ar: 'الامتثال',          href: '/governance' },
                    { en: 'Vision 2030 Alignment', ar: 'رؤية 2030',     href: '/governance' },
                    { en: 'Audit Reports',      ar: 'تقارير التدقيق',   href: '/governance' },
                  ].map(item => (
                    <Link key={item.en} href={item.href} onClick={() => setDropdown(null)}
            ...
            <div className="relative">
              ...
              {dropdown === 'dash' && (
                <div className="absolute top-full mt-1 left-0 w-52 rounded-xl border overflow-hidden z-50 py-1"
                  style={{ background: 'rgba(5,18,30,0.98)', borderColor: 'rgba(0,122,186,0.25)', backdropFilter: 'blur(12px)' }}>
                  {[
                    { en: 'Athlete Dashboard',  ar: 'لوحة الرياضي',    href: '/athlete',   icon: '🏃' },
                    { en: 'Facility Dashboard', ar: 'لوحة المنشأة',    href: '/facility',  icon: '🏟️' },
                    { en: 'Ministry Dashboard', ar: 'لوحة الوزارة',    href: '/ministry',  icon: '🏛️' },
                  ].map(item => (
                    <Link key={item.en} href={item.href} onClick={() => setDropdown(null)}
                      className="flex items-center gap-3 px-4 py-2.5 text-white/55 hover:text-white hover:bg-white/5 transition-colors text-[12px] font-cairo">
        ...
        <div className="max-w-3xl mx-auto w-full px-6 py-10 lg:py-16 flex flex-col gap-7 items-center text-center">
          ...
          <div className="flex flex-col gap-7 max-w-xl">
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {[
                { icon: 'USER_TRACK', en: 'Digital sport identity',      ar: 'هوية رياضية رقمية',         href: '/athlete' },
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                ...
                <span>{lang === 'ar' ? 'جرّب الآن' : 'TRY IT NOW'}</span>
              </button>
              <button
